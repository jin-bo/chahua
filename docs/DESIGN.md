...
上传成功后文件**不立即进上下文** —— 在 pending pill 上等。用户下一条 `user_message` 时前端把 `files: ["share/xxx.pdf", ...]` 一起带上，server 端 `_attach_files_to_text` 把每个 ref 拼成 `<attachment uri="share/xxx.pdf"/>` 自闭合标签单独一行追加到 text 后（与 agentao 视觉降级标签同格式；`mimetype` 仅图白名单扩展名给出）：

```
我看看这两份合同有什么不一样
<attachment uri="share/合同 A.pdf"/>
<attachment uri="share/合同 B.pdf"/>
```
...
| **P9 切房后房间后台续跑**（✓ 完工，详见 [docs/P9-切房后房间后台续跑.md](P9-切房后房间后台续跑.md)） | 切房从「cancel + 替换 + 关旧房」改为「仅切前台指针 + 重发快照」：server 从持 1 个 `_session` 升级为「多个 `RoomRuntime` 注册表 + 一个前台指针」；引入 per-room 可变路由 sink（`RoomEventRouter`），解决「sink 顺调用栈透传、in-flight turn 无法中途换 sink」的卡点；后台 runtime 仅在真有 in-flight 活或挂着 dormant MTS 时存在，跑完自毁、资源天然有界；`MAX_BACKGROUND_ROOMS = 5` 软上限。不改对话原语 / Task / handoff / MTS 数据结构，不引入跨重启持久化。 | 切走房间后原 turn / handoff drain / MTS 在后台续跑到自然完成；切回重发快照恢复；ws 真正断开才清后台 runtime；前台有 `isolation=global` 茶客切走前先 cancel+drain |
| **P10 聊天界面增强**（✓ 完工，详见 [docs/P10-聊天界面增强.md](P10-聊天界面增强.md)） | 气泡从「纯文本框」升级为「会渲染 / 会展产物」：① markdown ` ```mermaid ` 在 `message_end` 全文到位时渲染 SVG（手工 sanitize 不换 DOMPurify）；② 茶客 `task_write_artifact` 产出的图片在 originating 气泡末尾懒拉预览（占位 `<img>` + `download_file purpose=preview` 回包灌字节）；③ 其它扩展（含 SVG）挂可点下载 pill。后端只加「消息 → 产物」落盘表 `message_artifacts.jsonl`（`MessageArtifactRegistry`，五条工具线经 `_maybe_record_artifact_path` 写入）+ envelope 可选字段 `originated_message_id`，不改对话原语 / Task 数据结构 / 不 bump `schema_version`。 | mermaid 只在全文到位渲一次、流式半截不闪；图片按 rel 去重、live + history 不双挂；切房 / clear 清 pending preview |
| **P10.1 聊天富渲染：数学 / 化学 / 代码高亮**（✓ 完工，详见 [docs/P10.1-数学化学代码渲染.md](P10.1-数学化学代码渲染.md)） | P10 的**纯前端半相位**（后端零改、不 bump `schema_version`、CLI 不渲染）：`$…$`/`$$…$$` 公式（KaTeX）、其内的 mhchem `\ce{}`/`\pu{}` 化学式、带语言围栏块的语法高亮（highlight.js v11）。核心难点是 **marked 的 CommonMark 反斜杠转义会在 KaTeX 看到 TeX 之前吃掉 `\,`/`\\`** —— 故用行内扩展「转义前封箱」把整段公式收成 carrier span、KaTeX 在 DOMPurify 之后于 live DOM 上延后渲。`renderMermaidIn` 单钩子泛化为 `enhanceContent` 编排器（三类增强区域不相交、各自幂等）。| 公式 / 化学 / 代码渲染且流式期间不闪；`trust:false` + 不传 `macros`（宏不跨公式泄漏）；代码块内 `$` 保字面；无可渲内容零加载（懒）；打包含 `katex/dist/fonts/` |
| **P10.2 flint 数据图表渲染**（✓ 完工，详见 [docs/P10.2-flint 数据图表渲染.md](P10.2-flint%20数据图表渲染.md)） | P10.x 第三里程，同为纯前端：茶客回复里的 ` ```flint ` 块（[flint](https://github.com/microsoft/flint-chart) 语义图表规格 JSON）在渲染进程编译成 ECharts option 并渲成 SVG（37 图型）。安全 / 资源边界整体沿观澜 P4.20（两仓钉同一份 `flint-chart@0.4.1` + `echarts@6.1.0`）：入口白名单 + 拒 `data.url` + 生成式编译 + **键域**出口断言 + `svg`/`richText` + 三条资源闸 + 失败保留源码。两条茶话室独有机制：① **按气泡宽重编译 + 绘图区下限**（气泡比 wiki 栏窄约 40%，照抄会把绘图区压成条；硬地板存在 ⇒ 横向滚动是常规分支）；② **视口门 + 两道实例回收**（flint+ECharts 是茶话室第一个**有状态**渲染器，而 `room_info` 每帧整列表重建）。配套引进作者侧 skill `flint-chart-author`（flint 是新语言，模型不会自发产出合法块）。| 图渲出且注入 payload 落成 `<text>` 文本；拒 `data.url` 零外部请求；三档徽标可分辨；切房实例归零；node 45 条用例经 pytest 带起 + `npm run smoke:flint` 真 Electron 21 项 |
| **P11 后台 Agent（并行执行）**（✓ 完工，详见 [docs/P11-后台 Agent.md](P11-后台%20Agent.md)） | 同一房间允许多个茶客后台并行跑 agent run（用户 `/bg` 弹窗或茶客 `spawn_agent_run(s)` 工具指派）。前台 in-flight 之外新增 run 级运行态（`AgentRun` frozen dataclass + `BatchMessageSink`）；bg run 期间不流式刷聊天、完成时一次性 `message_end(ok)` 进 transcript（前端「无 start 直接 end」兜底渲气泡）；中途 `TASK_PROPOSAL` 缓冲到 finally flush；artifact 走 wrapper finally 的 detect 统一产出；`MAX_AGENT_RUNS_PER_ROOM = 4`；bg run 占用的茶客不接 `@` / 不参与打分（`guest_busy` vs `guest_in_bg_run` 二分）；MTS × bg run 续命按 spawn 时刻快照冻结。4 个 `AGENT_RUN_*` 事件不 bump `schema_version`。 | 多茶客同房并发 bg run；失败 / 取消只推终态不污染 transcript；5 步清理 race-free；bg run 不污染前台取证（`record_debug=False`） |
| **P12 persona 包 manifest（+ P12.5 / P12.6）**（✓ 完工，详见 [docs/P12-persona 包 manifest.md](P12-persona%20包%20manifest.md) / [P12.5](P12.5-MCP%20env%20变量插值.md) / [P12.6](P12.6-Personas%20更新.md)） | persona 从「目录散件约定」升级为带 `persona.toml` manifest 的安装单元：顶层 `schema_version`（当前唯一合法值 =1）+ `display_name` / `summary` / `[defaults.guest]`，三级严格白名单、坏 manifest 消费路径 fail-fast（`discover_personas` 是唯一 WARN+None 例外）；`[defaults.guest]` + 顶层 `summary` 仅「加入房间」时一次性 inflate（之后与 `room.toml` 解绑）。**P12.5**：persona / 房间级 `mcp.json` 的 `env` / `headers` / `args` 支持 `$VAR` / `${VAR}` 插值（真密钥留 `.env`，把「API key 永不进 toml/envelope」延伸到「永不进 persona 包」）。**P12.6**：导入补来源记录（`.chahua-source.json` provenance，绝不进 `persona.toml`）+「已安装」页 + 检查/更新（全量替换 + 原子 swap、本地改动检测 + `force`、status 只由 SHA / content_hash 定）+ 删除（路径强校验防穿越）。 | 写 `persona.toml` 后 picker / `add_guest` 按 manifest 默认装配；mcp.json 不再硬写密钥；已安装页列受管 persona、检查更新按内容信号定 status、更新原子替换不留半个 persona |
| **P13 多模态图像输入**（✓ 完工，详见 [docs/P13-多模态图像输入.md](P13-多模态图像输入.md)） | 用户发图，视觉能力的茶客模型直接看像素、非视觉茶客自动退化为文本引用（逐茶客独立，因每个 `TeaGuest` 包独立 `Agentao` / model）。降级零自研归 agentao reactive 回退，chahua 只把 `images=[{data, mimeType, _source}]` 传进 `arun()`、不维护 per-provider 视觉能力表。视觉附图纯瞬态：`images_rel` 沿当前 turn 同步透传（`submit_user_message → _run_ai_chain → let_speak → speak`）、base64 懒读不入库，不改 `Message` / transcript / envelope / `schema_version`；transcript 只留 `<attachment uri="share/.." .../>` 文本标记（与 agentao 降级标签同格式）。附图范围严格 = 本轮触发用户消息且只进 `_run_ai_chain` 第一周期；打分永不吃图。 | 视觉茶客看图、非视觉茶客退文本引用；图类型扩展名白名单 `{png,jpg,jpeg,gif,webp}` + MIME 映射；读盘双层防穿越 + 0 字节 / >20MB / >16 图跳过截断 |
| **P14 系统 prompt 英文化 + 精练 + 发言前回顾历史**（✓ 完工，详见 [docs/P14-系统-prompt-英文化与发言前回顾历史.md](P14-系统-prompt-英文化与发言前回顾历史.md)） | 把所有「系统生成、喂给 LLM」的 prompt 文案（context 块 / 功能块 / task 渲染 / 工具 description+return / 便宜模型 system prompt）由中文改英文并逐块精练（去 rationale / 去重复 / 散文转祈使），收 token 下降 + 指令遵循提升双红利。用户内容（persona / USER.md / 房间 topic·rules / 聊天本身）一律不动，输出语言随对话（默认中文，靠每轮塑造块的语言锚点句）。`_speak_instruction_block` 追加 recall 段落：发言前回顾自身 `agent.messages` 的 tool 结果、仅截断/缺失才回 source 重读。纯文案改动，不动结构 / schema / 形参 / 事件类型 / 功能性字面量。 | 系统 prompt 英文、输出仍随对话（默认中文）；精练不丢承重语义、不改工具名 / 路径 / `Error:` / XML 标签·属性 / status 枚举 / JSON 字段；MTS `<managed_session>` 5 条指令顺序不变 |
| **P15 桌面登录态自动注入 LLM 配置**（✓ 完工，详见 [docs/P15-桌面登录态自动注入 LLM 配置.md](P15-桌面登录态自动注入%20LLM%20配置.md)） | 用户在 desktop 登录后拿到 provider URL + apikey 并选模型，**自动注入 chahua、零重启生效**，不必手改 `.env`。复用既有 `_replace_session()` 热重建，只加一个触发入口 + 凭证注入点：`set_llm_credentials` inbound（前台房专用，严格白名单 `{provider, model, base_url?, api_key}`）→ 只改 `os.environ` → cancel 全前台 → 热重建。**纯 env、绝不写 toml、绝不进日志**；改 env 前快照旧值，失败逐个回滚 + 保留旧 session；粒度只动「房间默认」，显式 LLM section 钉住。 | 选模型后下一轮即用新模型且无重启；raw key 只活在进程 environ（`api_key` 校验 `redact=True`、`base_url` 日志剥到 `scheme://host`）；注入失败原样回滚 |
...
- **2026-08-04（§6 分阶段表回填 P10.1 / P10.2 / P15–P18）** —— 上次回填停在 P14 行（2026-06-03），
  ...
  `P10.x` 纯前端呈现层子序列，故插在 P10 行之后）/ P15 桌面登录态注入 LLM 配置 / P16 发言权重与
  手动模式 / P17 只读长期记忆（GuanLan MCP）/ P18 消息回溯与历史搜索。**P18 标「部分完工」**
  （M1 搜索 + 撤回已落地，M2 全局 / 正则搜索仍为设计），**P17 注明 P-mem.3 / P-mem.4 暂缓** ——
  表里不写「✓ 完工」以外的含糊状态，宁可把边界写在标题里。本次同样只动 §6 表与 §8，延续
  「早期架构 + 阶段索引」定位。

- **2026-06-03（§6 分阶段表回填 P9–P14）** —— 自 2026-05-21 回填 P8 后，§6 表停在 P8 行 +
  ACP 占位行，而其后已陆续落地：P9 切房后房间后台续跑（多 RoomRuntime + 可变路由 sink）/
  P10 聊天界面增强（mermaid / 图片预览 / 下载 pill）/ P11 后台 Agent（同房并行执行）/
  P12（+ P12.5 / P12.6）persona 包 manifest、MCP env 插值、Personas 更新 / P13 多模态图像
  输入 / P14 系统 prompt 英文化与发言前回顾历史。本次只动 §6 表（补 6 行，各指向
  `docs/Px-*.md` 专项文档）与 §8，延续「早期架构 + 阶段索引」定位 —— **不在 §3 展开这些
  阶段的完整设计**，关键设计点与不变量见各专项文档及 `docs/INVARIANTS.md` / `CLAUDE.md`。

- **2026-05-29（§9 不变量明细移出到 `docs/INVARIANTS.md`）** —— CLAUDE.md「关键不变量」一节
  本轮精简为「规则 + 一句 why」；§9 完整底本（rationale / P-版本溯源 / 回归测试）移到独立的
  `docs/INVARIANTS.md`，§9 原位留指针。DESIGN.md 回归「早期架构 + 阶段索引」定位，不变量明细
  自此独立成文。同步改 `CLAUDE.md` 段首指针。
...
> 已移出到独立文件 **[`docs/INVARIANTS.md`](INVARIANTS.md)** —— CLAUDE.md 不变量的完整底本（完整 rationale / P-版本溯源 / 实现细节 / 回归测试文件名）。
> 改动不变量时，`CLAUDE.md` 精简条目与 `docs/INVARIANTS.md` 明细两处同步更新。
