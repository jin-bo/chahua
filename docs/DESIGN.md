...
| **P8 任务管理：skill 形态与原生自动推进**（✓ 完工，详见 [docs/P8-任务管理 skill 与原生化.md](P8-任务管理%20skill%20与原生化.md) / [P8.3](P8.3-原生自动推进.md)） | 让房间「被管理」：P8.1 `examples/personas/Maya/skills/task-management/SKILL.md` 茶客侧任务管理工作手册（零后端改动、跑在 P5~P7 原语上、随 Maya persona bundle）+ Maya 管理者 persona；P8.2 第五个 task 工具 `task_propose_status`（提议改任务状态，采纳按终结态分流 `update_task` / `close_task`）+ 能力花名册（`[[guest]].summary` room.toml 字段 + `persona_summary.py` LLM 生成的中央内容寻址缓存，onboarding `<room>` 块渲 bullet 花名册）；P8.3 托管任务会话（MTS）—— 用户显式开启、有预算上限，管理者茶客在 P7 的 handoff drain loop 上自动「指派 → 执行 → 复查 → 再指派」推进任务，复用既有调度层零新增路径。 | 装 SKILL.md 的茶客能拆任务分工；`task_propose_status` 采纳后状态正确落库；onboarding 见花名册能力摘要；开启 MTS 后管理者自动推进、预算 / cap 耗尽收尾、可一键停止 |
...
- **2026-05-21（§6 回填 P8 + §9 补 P8.2 花名册 / P8.3 MTS 不变量）** —— P8 任务管理
  阶段落地：P8.1 task-management skill（茶客侧工作手册 + Maya 管理者 persona）/ P8.2
  `task_propose_status` 工具 + 能力花名册 / P8.3 原生自动推进「托管任务会话（MTS）」。
  §6 分阶段表加 P8 行（指向 `docs/P8-*.md` / `docs/P8.3-*.md` 专项文档），§9 补 P8.2
  能力花名册与 P8.3 托管任务会话的关键不变量。完整设计仍在各 `docs/Px-*.md`，DESIGN.md
  维持「早期架构 + 阶段索引 + 不变量明细」定位。
...
## 9. 关键不变量明细（CLAUDE.md 移出）

> CLAUDE.md 的「关键不变量」一节为节省每次会话的 context 已精简为「规则 + 一句 why」。
> 本节保留各条的**完整 rationale / P-版本溯源 / 实现细节**作为参考底本，按 CLAUDE.md 原始顺序排列。
> 改动不变量时，CLAUDE.md 精简条目与本节明细应同步更新。
