...
| **P3.2.2 茶客侧栏 + @ 补全** | wire 加 `room_info` 出站帧（ws 连上即下发：room name + 茶客名单 [name, permission, isolation 占位] + user display_name）；renderer 渲染 sidebar；输入框 `@` 触发候选 dropdown；选中插入 `@<名字>` 字面量（服务端 `_route` 已识别） | sidebar 有人；@ 弹候选；选中后 `_route` 命中 |
