...
- **托管任务会话（MTS）是瞬态运行态，单房间最多 1 个，不落盘**（P8.3）。`ManagedSession`（`task_id` / `manager_guest` / `budget`）挂 `Orchestrator._managed_session`，与 `_handoff_queue` 同瞬态语义——不落盘、crash / `reset_room` / ws 断开即清，不跨重启恢复。**P9 起切房不结束 MTS**：旧前台 busy 转后台续跑，MTS 在后台自驱到 budget/task/cap 自然收尾；只有 ws 真正断开（`_serve_one` finally `_aclose_background_runtimes`）才强制结束后台 MTS。`_managed_session is None` 即「无托管」，**不**另存 `enabled` bool（避免「`enabled=False` 但对象还在」的二义态）。只经 `managed_session_start` inbound 开启——无自动 / 超时 / 茶客 propose 开启（MTS 授权茶客在预算内自主烧 token，必须用户显式确认）。`MAX_MANAGED_BUDGET = 20`（入站上限硬护栏）/ `MANAGED_SESSION_DEFAULT_BUDGET = 6`（前端预填）是 `handoff.py` 模块常量、不进 toml。
