...
| **P2.2 事件 envelope** | `ChahuaTransport`（`SdkTransport` 子类）合成 `message_start` / `message_delta` / `message_end`（含 status / partial_text）；orchestrator 合成 `turn_start` / `turn_end`；`CancellationToken` 接入 `speak()`；CLI 仍是消费者（把 envelope 渲染回打字流） | 异常 / 取消 / 部分输出有明确边界；message_start 必有对应 message_end |
