...
# P5.2 mixin 拆分：``GuestSnapshot`` / ``TomlSnapshot`` 视图类型 + room.toml 写出层
# 都在 ``.admin_toml``；本模块 ``from .admin_toml import GuestSnapshot, TomlSnapshot,
# write_room_toml, _extra_mcp_dict_to_list, _llm_spec_to_dict``（见顶部 import）。
