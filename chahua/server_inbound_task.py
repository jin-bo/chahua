"""任务房间 inbound + emit_task_* mixin（P5.2 重构，docs/P5-任务房间.md §7.2）。

P5.1 期间 ``server.py`` 单 ``ChahuaServer`` 类承担 2100+ 行 / 30+ ``_inbound_*``。P5.2
按 inbound feature 切到 4 个 mixin —— 本文件持任务房间四个 inbound（open_task /
update_task / attach_artifact / add_decision）+ 两个 task envelope 助手。

mixin 鸭子类型：依赖 ``self._session`` / ``self._emit_notice`` / ``self._reject_unknown_keys``
/ ``self._notice_persist_failure`` 由核心 :class:`chahua.server.ChahuaServer` 提供。
"""
...
# 任务 inbound type 常量。集中放在 mixin 顶让 "type 字面值" 也跟随实现走。``server.py``
# 的 ``_INBOUND_HANDLERS`` 表 import 这几个常量做 wire 字符串映射。
...
class TaskHandlersMixin:
    ...
    与 ``ChahuaServer`` 通过 MRO 合并 —— 所有 ``self.xxx`` 引用在合并后的类实例上生效。
    本 mixin 不持状态、不定义 ``__init__``，纯函数集合。
    """
