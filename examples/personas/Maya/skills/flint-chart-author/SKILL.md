...
字段名用**数据里的原名**（中文列名完全可以）。**想改图上的显示名，就在内联数据里把那一列改成你要的名字**
——`field_display_names` 这个键**实测不生效**（`flint-chart@0.4.1` 的 dist 里根本没有它，`assembleECharts`
只读 `chart_spec` / `semantic_types` / `data.values` / `options`）；它留在合法顶层键清单里只是向前兼容，
写了等于没写。
...
- [ ] 若写了 `baseSize`：宽高都是**有限正数**，且**放大后**不超过 **1600 × 1200**
      （注意：`baseSize` 是**基准不是上限**——flint 会按类别数/分面数把它**放大**，实测 `420×260`
      在多序列下会长到 `657×514`。渲染端卡的是**放大后**的尺寸，所以类别特别多时**减类别**——
      别指望 `canvasSize` 兜底，它是**收窄提示不是硬钳位**，产物恒比请求值大几十像素，
      请求值高过自然尺寸时还会被整个忽略，见参考第 1 节）
- [ ] **画布没有下限**：按语义定尺寸的小图（如两周的 `Calendar Heatmap` 只有百来像素宽）完全合法，
      不要为了凑一个「最小尺寸」硬把图撑大——小画布不消耗任何资源，渲染端也不拦它
