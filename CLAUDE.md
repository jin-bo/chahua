...
- **`managed_session_start` 拒收 busy manager（P8.4.7）**。`_inbound_managed_session_start` 在 ②（manager 在场）和 ③（budget 范围）之间加 ②b：`manager_guest in orch.active_guest_names` → NOTICE error 拒开。**Why**：kickoff DELEGATE 会被 drain `_advance_to_runnable_handoff` 的 busy-winner 守卫静默 drop（P11 C12），而 P8.4「队列空→dormant」让 UI 已收到 `managed_session_started` 却永远没 kickoff 接续，MTS 卡死无反馈（Codex P8.4 round 1 P2）。busy 名单读 `active_guest_names` 视图（与 drain busy 守卫同源）。
