...
- `orchestrator.py` + `_orchestrator_{chain,handoff_drain,handoff_queue,managed_session,scoring,consts}.py` — 意愿打分主循环。流程：房间新消息 → 对每个空闲茶客并发跑轻量 LLM 打分（裸 LLMClient，不走完整 Agentao）→ 取分数 ≥ `want_threshold`（0.45）的前 1~2 名真正发言 → 没人过阈值就等用户。`max_consecutive_ai_turns` 截顶；`@<名字>` 走确定性路由不进打分。**slot 重构**：主类 `Orchestrator` 按子域拆 5 个 slot 模块（模式仿 `server.py`/`admin.py`，组合而非多继承）：`AIChainOps`（`_run_ai_chain` + 3 个 turn 级 emit）/ `HandoffDrainOps`（`run_pending_handoff` + winners/cost/渲染算法）/ `HandoffQueueOps`（`_handoff_queue` + `enqueue/clear/has_pending` + 2 个 emit）/ `ManagedSessionOps`（`_managed_session` + MTS 生命周期 / 推进 / proposal 拦截）/ `ScoringOps`（`pick_next_speaker` / `score_one` / `find_user_mention` / `find_user_broadcast` / `count_self_mentions` / `let_speak`）。`_install_orchestrator_slots(orch)` 单点装配；公开 import 路径不变（`from chahua.orchestrator import Orchestrator / OrchestratorConfig`），主类经 property + 薄转发覆盖外部直访（含 30+ 测试 `orch._handoff_queue` / `orch._managed_session` / `orch._find_user_mention()` / `Orchestrator._handoff_cost(...)` 等）。**主类必须保留 `_run_ai_chain` / `_intercept_task_proposal` 同名 method**：前者被 `tests/conftest.py` / `test_orchestrator_run_pending_handoff.py` `monkeypatch.setattr(Orchestrator, ...)` 替换；后者经 `session.py:511 guest.set_task_proposal_hook(orchestrator._intercept_task_proposal)` 取 bound method 引用。
...
- **`run_pending_handoff` 与 `_run_ai_chain` 严格分流，不互相回落**。drain loop 队列空 / cap 撞顶就停 + emit `turn_end(next="user")`，不回落 scoring；下次 scoring 须用户消息触发。`@A` 完后回 scoring、`/delegate A` 完后不回落。（slot 重构：实现分别在 `HandoffDrainOps.run_pending_handoff` / `AIChainOps.run_ai_chain`，主类同名薄转发；`submit_user_message` 是唯一编排两者的入口，先 drain 再 chain。）
- **drain loop 每轮 turn 末尾 5 步严格对齐 `_run_ai_chain`**：① peek 算 `cost` 比 cap 得 `next_state` → ② 一次性 `turn_end(next=next_state)` → ③ `flush_turn` → ④ `_kick_summarize` / `_tick_cooldown` / `_kick_detect_new_artifacts` → ⑤ `if not has_next: return`。三个 hook 不能放 `turn_end` 之前、要么一起加要么一起删。（slot 重构：5 步实现位于 `HandoffDrainOps.run_pending_handoff`；hook 走 `self.orch._kick_summarize()` 等经主类，主类内仍是原方法。）
...
- **handoff envelope emit 职责拆分；`reason` 不进茶客 prompt**。`enqueue_handoff` / `clear_handoff_queue` 纯方法不 emit；`handoff_enqueued` / `handoff_cleared` 由 server handler emit、`handoff_consumed` 由 orchestrator 在 `TURN_START` 之后 emit。`HandoffItem.reason` 是内部备注，永不进 prompt。（slot 重构：`enqueue/clear/has_pending` 在 `HandoffQueueOps`；`handoff_consumed` 由 `HandoffDrainOps.run_pending_handoff` 经 `self.orch._handoff_ops.emit_handoff_consumed(...)` emit。）
...
- **panel `cost` 两档 cap 检查 + 跑不起来的队首项就地 drop**。`_advance_to_runnable_handoff` 是 drain 主体与 `has_next` lookahead 共用 helper：弹掉死项（cost > max / panel 欠员 / target 删光）、每弹一项 WARN + 重发 `HANDOFF_ENQUEUED`。撞预算的项不弹、`break` 等下次；死项必 pop 不能 break。（slot 重构：`_advance_to_runnable_handoff` / `_handoff_cost` / `_panel_underfilled` / `_resolve_handoff_winners` / `_build_winner_blocks` 均在 `HandoffDrainOps`；`_handoff_cost` 是 `@staticmethod`，主类同名 `@staticmethod` 薄转发兼容 `Orchestrator._handoff_cost(...)` classmethod-style 测试调用。）
...
- **MTS 跑在 handoff drain loop 上，不新开调度路径**。`run_pending_handoff` drain 每轮 turn 跑完调一次 `_advance_managed_session_after_turn(item, sink)`，再照常走既有 peek / turn_end / 收尾，既有「5 步」内部顺序一字不动；`_managed_session is None` 时该调用立即返回，非 MTS 房间零行为变化。`_advance` 按「刚跑完的是不是管理者回合」分流：管理者回合不做事（其 delegate/panel 提议已被 hook 入队）；worker 回合且队列空 → `budget-=1` + 回调 `delegate(manager)` + emit `managed_session_advanced`。停止守卫 `_managed_session_stop_reason()` 先判（budget → task_closed → cap，谁先命中报谁）。（slot 重构：状态 `_managed_session` 和 `advance_after_turn` / `stop_reason` / `intercept_task_proposal` / `start/end_managed_session` / `emit_managed_session_snapshot` 等 12 方法均在 `ManagedSessionOps`；主类经 `_managed_session` `@property` (含 setter 兼容测试直赋) + 同名薄转发暴露。drain slot 经 `self.orch._mts_ops.advance_after_turn(...)` / `end_managed_session(...)` 调；读 MTS 状态经 `self.orch._mts_ops._managed_session`。MTS 写队列经 `self.orch._handoff_ops` —— **不直接持 queue ref，MTS 是 queue 的客户、不是所有者**。）
