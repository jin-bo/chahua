...
- **bg wrapper finally 5 步**：`detect_new_artifacts` → `BatchMessageSink.flush_to(router)` → `agent_runs.pop(run_id)` → emit `AGENT_RUN_{FINISHED,CANCELLED,ERROR}` → `active_guest_names.discard(guest_name)` → `_maybe_self_destruct_background_runtime`。**terminal envelope 必在 propose flush 之后**；**自毁必在 discard 之后**（防读到残留）。任一 step 抛异常仍保证后续 step 执行。
