...
- **permission / isolation 默认值合一仅在 admin 层；frontend → inbound → admin 全链路用 `None` 表示「用户未显式选」**。frontend `modals.js` 不送 `permission`、inbound `_inbound_add_guest` / `_inbound_create_room` 保留 None、admin `_build_guest_with_manifest_defaults` 三级 coalesce（入参 > manifest > `DEFAULT_MODE`）一律 `is not None`，**禁** `or DEFAULT_MODE`（`or` 会把 `""` 这类显式坏值静默吞，用户 typo 永不可见）。`_render_room_toml` / `config.py` 仍存 `or DEFAULT_MODE` 是 P12 前既有行为，breadcrumb 测钉住。
- **消费路径严格 fail-fast；`discover_personas` 是唯一 `WARN+None` 例外**（picker 可见性优先 —— 不让坏 manifest 把整个 persona 从列表抹掉）。`add_guest` / `create_room` / `persona_import` 遇坏 manifest → `PersonaManifestError` 向上抛 → inbound `_emit_notice(level=error)`；`create_room` 事务性：所有 guest 的 manifest 先解析成功才 `mkdir(exist_ok=False)`。`persona_import._validate_manifest_pre_write` 在 `_write_files` 之前对采集 files 里**根级小写 `persona.toml`** 字节做 dry-run（`Persona.toml` 等错名同样拒，跨平台 case-insensitive FS 旁路守卫）。
- **仅 dir-form 才查 manifest，flat-form 跳过**。flat-form md 的 parent 是 `chahua/personas/` 本身；误置一份根级 `persona.toml` 会让所有 flat-form persona 共享 `[defaults.guest]`。两处守卫同语义不同写法：`admin_room._build_guest_with_manifest_defaults` 内联判定 `md_abs.parent.name != PERSONAS_REL_DIR.name`；`admin_persona._resolve_display_and_summary` 接 `is_dir_form: bool` 形参。**P12.1 起**：内置 5 位迁到 dir-form + manifest；flat-form 仍是合法格式（社区包可用）。
- **P12.1 backward-compat：load_room_config 自动把 flat-form built-in 路径升级到 dir-form**。pre-P12.1 老 room.toml 里 `persona = "chahua/personas/宝总.md"` 仍可加载 —— `config.py::_try_p12_1_dir_form_rewrite` 在 `find_in_data_then_app` miss 时尝试 dir-form 路径 + WARN 一次。建议用户改 toml；不强制。Shim 后续 phase 可删（届时 pre-P12.1 用户已升级）。

### Personas 更新（P12.6）

承重契约见 `docs/P12.6-Personas 更新.md`「承重不变量」段。改不变量两处同步。

- **provenance 是消费侧安装元数据，住 `.chahua-source.json`，绝不进 `persona.toml`**。manifest 是作者可分发清单（更新被上游覆盖）；来源 / commit sha / 安装时间是「装它的人」的本地事实（npm `package.json` vs `package-lock.json` 分层）。`.chahua-source.json` 进 `_SKIP_NAMES`（采集三处都跳：`_walk_local` / GitHub walker / installed-dir 哈希遍历经 `_collect_local_files`）—— 不跟包漂走、更新时由 importer 新鲜写。
- **provenance 读容错（唯一允许 `try/except + WARN + 降级` 的 persona 路径）**。`read_source` 缺文件 → None；坏 JSON / `schema_version`≠1 / 字段不合法 → WARN+None。坏 provenance 不让 persona 从「已安装」列表 / picker 消失，只去掉「更新」能力、保留「删除」（标 `status="source_unavailable"`）。**与 `persona.toml` 的 fail-fast 口径有意相反**。
- **`persona.toml` 的 `version` 是纯展示字段，绝不参与 `status` 判定**。「变没变」永远只由 commit SHA（github）/ content_hash（folder）说了算。**内容变了一律 `update_available`，即使上游版本号更低（降级）也照样提示**。**不做 `_cmp_version`、不解析 semver**（测试钉死无该符号）。`latest_version` 取数失败（404/网络/解析/缺字段）只是 `latest_version=null` + detail 提示，**绝不改 status**（version 取数独立 try/except，与定 status 的内容信号检查隔离）。`version` 是 manifest 新增**可选**字段，加它**不** bump `schema_version`。
- **更新 = 全量替换 + 原子 swap，绝不原地改写**。`_replace_dir_atomic`：tmp dir 写新内容 **+ provenance 同次换入** → `rename(target→bak)` → `rename(tmp→target)` → 成功删 bak / 失败 restore。manifest dry-run 由 `update_persona` 在 swap **之前**做（坏 manifest 旧版完好）。tmp/bak 名带 `pid + token`（`update_persona` 经 `to_thread` 跑，防同进程并发更新同一 persona 撞名 / 互删工作目录）。**Why**：半个 persona 比旧 persona 危险得多。
- **更新前本地改动检测；本地已改 → 默认拒，须 `update_persona{force=True}` 才覆盖**。安装存 content_hash，更新前重算已安装目录哈希比对。`force` 把「确认丢弃本地改动」的决定钉在服务端（单靠前端 confirm 不够 —— wscat / 旧前端可绕过）。哈希一处算（`_content_hash`）、三处复用（安装记录 / 本地改动检测 / 文件夹源 diff）。`status` 与 `local_modified` 两正交维度。
- **更新 / 删除不影响在跑的房间，下次 session 重建才生效 / 才因路径缺失报错**。已构造的 `TeaGuest` 不受磁盘改动影响。不主动扫房间 toml 拦截删除 —— 前端 `confirm()` 文案提示，交给既有 session 重建边界。
- **「已安装」列表 = `user_data_root` 下所有 dir-form persona；provenance 只 gate「检查/更新」，不 gate「列出/删除」**。内置 flat-form（app_root 只读）不进列表；`list_installed_personas` 跳 dot-dir（原子替换瞬态残骸）。`name` 字段 = **目录名**（update/delete/check 的操作键，经 `_user_persona_dir` sanitize + `relative_to` 防穿越解析回该目录），`display_name` 友好名渲染用。
- **删除只 `rmtree` 一个 `user_data_root` 下的 persona 目录，路径强校验防穿越**。`_user_persona_dir` 走 `sanitize_fs_name` + `relative_to(user_data_root/chahua/personas)` + 必须 `is_dir()`；app_root 内置天然不在 user_data 根内 → 拒。
- **检查 / 更新（网络型）走 `asyncio.to_thread`，不阻塞 WS 事件循环**。`check_persona_update` 内部吞掉所有「预期失败」（源已删 / 404→`source_unavailable`、403/网络→`error`、取版本号失败→version=null）映射成 `status`，只在真正意外时才抛（inbound `_check_one_into_row` 兜底成 `error` 行，不中断其余行）。`_GitHubError` 子类化 `PersonaImportError` 带 `.code` 供分流。import 时取 commit_sha best-effort（失败存 None 降级，不回退导入）。
- **`PERSONAS_INSTALLED` 是权威全量快照，按需发，绝不进 `room_info`**。`list` / `check` / `update` / `delete` 后均回这一帧（前端整批覆盖）；provenance 读盘 + 网络检查只在用户开「已安装」页 / 点「检查更新」时发生。4 个新 inbound + 1 个 envelope **不 bump `schema_version`**（沿用旧前端忽略未知 type）。
...
- **后台 runtime 仅在真有 in-flight 活或挂着 dormant MTS 时存在**。bg turn / handoff drain / bg run 跑完，wrapper finally 调 `_maybe_self_destruct_background_runtime` 自毁。**P8.4 后** `busy_alive` 把 `has_managed_session()` 计入 busy —— dormant MTS 让 bg runtime 留活（用户切回还能看到 MTS 状态）；正经收尾（`managed_session_stop` / task 终结 / `_aclose_one_runtime` / `MAX_BACKGROUND_ROOMS` 淘汰）都先 `end_managed_session()` 再 close，故不泄漏。切回竞态：`_switch_room` 先翻 `mode=foreground`，自毁判定随即不成立。
...
- **停止条件 —— P8.4 起 5 个 reason**（`MANAGER_FINISHED` 已退役）：`budget_exhausted` / `task_closed` / `cap_reached` / `user_stopped` / `user_cancel`。终结只能因 ① 有界资源（budget / task / cap）或 ② 用户显式行为（stopped / cancel）。`task_closed` 触发：关 MTS 任务、用户采纳 `task_propose_status("done")`、或开新任务 / 切走 active 让 MTS 任务不再 active（`stop_reason()` 含 `active_task_id != ms.task_id` 检查）。管理者「我已完事」走 `task_propose_status("done")`，不另设 reason。
- **drain 收尾队列空 + MTS 活 → 保持 dormant，不终结**（P8.4 §3.1）。`run_pending_handoff` body 内 `has_next=False` 与 while-break 两处都 `return` 不调 `end_managed_session` —— 管理者没派活 ≠ 事情结束（思考 / 暂停 / 等用户输入 / 卡壳）。
- **dormant 复活走 drain 路径 + skip chain（P8.4.7）**。`submit_user_message` 入口当 MTS 活 + 队列空 + manager 在场 + 不忙时，**pre-enqueue** 一个 manager DELEGATE kickoff + emit `HANDOFF_ENQUEUED` 快照 → 既有 drain 像 `managed_session_start` 路径一样消费它（`<managed_session>` 块经 `_build_winner_blocks` 注入）→ 该路径**跳过** `_run_ai_chain`。**Why**：原 P8.4.1「chain 跑完 → re-drain」依赖 manager scoring 胜出 + 拿 `<managed_session>` 块，两者都不成立（scoring 是统计行为、`<managed_session>` 块是 drain 独占注入点），dormant 永远卡死。manager 不在场 / 正忙 → 不补 kickoff、走常规 chain（MTS 仍 dormant 等下次）。chain 后 re-drain（`reset_cap=False`）保留：捕获 chain 内被 `@` 强制 / 偶然胜出的 manager 经 hook 自动入队的项。
- **`managed_session_start` 拒收 bg-run busy manager（P8.4.7 / P8.4.9）**。`_inbound_managed_session_start` 在 ②（manager 在场）和 ③（budget 范围）之间加 ②b：`runtime.guest_in_bg_run(manager_guest)` → NOTICE error 拒开。**Why**：kickoff DELEGATE 会被 drain `_advance_to_runnable_handoff` 的 busy-winner 守卫静默 drop（P11 C12），UI 收到 `managed_session_started` 却永远没 kickoff 接续，MTS 卡死无反馈。**P8.4.9 narrow**：只查 bg run、不查 `active_guest_names` 全集 —— `_enqueue_handoff_and_maybe_start` 会 cancel + drain user/handoff inflight 抢占清场，前台 / handoff 占用 manager 不阻止启动；只有 bg run 不可抢占。
- **MTS proposal intercept：summarizer 不查 manager 自己（P8.4.9）**。`ManagedSessionOps._handoff_item_from_proposal` 在 manager 自己的 `speak()` 内跑，manager 此刻标 busy；但提议的 handoff item 在 manager `speak()` finally 解 busy 后才被 drain 消费。「workers 讨论、我汇总」（panel.summarizer = manager）是 MTS 核心模式 —— summarizer 用 `busy - {manager}` 检查放行；delegate target=manager / panel panelist=manager 仍按全集 busy 拒（圆桌应为 worker 间讨论）。
...
- **自指派 early swallow（P8.4.11）**。`_intercept_task_proposal` 调下游 `_handoff_item_from_proposal` 之前先识别 `kind=handoff_delegate ∧ payload.target == ms.manager_guest` → 直接 swallow + return True。**Why**：`let_speak` 已标 manager busy → 下游 delegate 全集 busy 校验返 None → 本入口退化「渲卡兜底」分支 → 用户点采纳真 enqueue manager，破坏「自指派不入队」语义。item-非空路径的自指派兜底保留（防下游未来改回返非 None）。

### 后台 Agent（P11，bg run / 并行执行）

- **`active_guest_names` 是 `guest_busy()` 唯一数据源**。`runtime.active_guest_names` 与 `orchestrator.active_guest_names` 是同一 set（经 `_attach_runtime_state` 注入）。**add 必先于任何 `await`**：bg run inbound 校验通过即同步 `agent_runs[run_id]=run` + `active_guest_names.add(target)`（先于 `create_task`）；前台 / handoff `let_speak` 在 `await guest.speak(...)` 外包 `try: add → finally: discard`。裸构 Orchestrator 时 `active_guest_names is None` 整段跳过。
- **`guest_busy` vs `guest_in_bg_run` 二分（P8.4.9）**。`guest_busy(name)` 读 `active_guest_names` 全集 = 「此刻占用」（前台 / handoff / bg run 任一）；`guest_in_bg_run(name)` 只查 `agent_runs.values()` = 「不可抢占」。**handoff inbound 准入校验（delegate / review / panel / managed_session_start ②b）必须用 `guest_in_bg_run`** —— 原 P11 C12 用 `guest_busy` 把「正在前台讲话但可被抢占」误判为「不可派活」，破坏 `_enqueue_handoff_and_maybe_start` 的 cancel + drain 抢占契约。**drain 自身的 busy-winner 守卫**（`_advance_to_runnable_handoff`）仍用 `active_guest_names` 全集兜底（覆盖 inbound→drain race / MTS auto-enqueue 瞬态）。
...
- **`busy_alive() = inflight_alive() or has_active_runs() or pending_mts_continuations or has_managed_session()` 仅 P9「runtime 生命周期 / 房间 busy 展示」分支用**。3 处替换：`_switch_room` demote / `_maybe_self_destruct_background_runtime` / `_rooms_available_with_busy`。**不用于前台 turn 控制** —— `_run_turn` / handoff drain 的 cancel/drain 仍走 `inflight_alive()`。P11.2.X 加 `pending_mts_continuations`（deferred 续命 callback 等住 self_destruct）；P8.4.8 加 `has_managed_session()`（dormant MTS 阻 bg runtime 销毁）。
...
- **MTS × bg run 续命 —— spawn 时刻快照（P11.2.X）**。`AgentRun.mts_managed` + `mts_manager_at_spawn` 由 `_start_agent_run` 在 spawn 时刻按 `(MTS 活, source_guest == ms.manager_guest)` **冻结**，不跟随后续 MTS 状态；manager 身份冻结防 MTS 换主后把复查塞错人。`_start_agent_run` 拒 `target == ms.manager_guest`（防 bg 内 propose 经 intercept hook 污染 MTS 队列）。
- **续命触发只认 `AGENT_RUN_FINISHED`**。wrapper finally 第 ⑤ 步仅 finished 走 `_continue_mts_after_bg`（cancelled / error 终态跳过）；helper 调 `ManagedSessionOps.advance_after_bg_completion(mts_manager_at_spawn=...)`：MTS 仍活 + 当前 manager 与冻结值一致 + `stop_reason` 不命中 + budget>0 → `budget-=1` + enqueue `DELEGATE(target=manager)` + emit `managed_session_advanced` 返 True；任一守卫不过返 False（stop_reason 命中走 `end_managed_session(reason)`，budget<=0 走 `end_managed_session(BUDGET_EXHAUSTED)`）。
- **续命后按 inflight 分流起 drain**。无 inflight → 起 `_run_handoff_turn` drain；handoff drain 已跑 → 不动；user turn 在跑 → `inflight_task.add_done_callback` 等 user 跑完起 drain（避免 stranded delegate）。`create_task` 失败 → `end_managed_session(user_cancel)` 兜底防悬挂。`run_pending_handoff` 收尾兜底两处 MANAGER_FINISHED 触发都加 `_has_pending_mts_bg()` 守卫。`/clear room` 与切房 global-guest 分支显式 `_maybe_end_managed_session(user_cancel)` 前置防 step ⑤ 抢跑。
- **budget 语义扩展**：「manager→worker（含 bg）→manager 桥扣 1」—— spawn N 个 bg ≈ 用 N budget；同回合 spawn + propose(worker) 会双扣（worker 桥 + bg 桥各 1）。
- **`_has_pending_mts_bg` orch ↔ runtime 严格 1:1**。该 bound method 经 `_attach_runtime_state` 绑到 orchestrator —— 未来若 session 跨 runtime 复用必须同步重绑，否则旧 orch 闭合旧 runtime → drain 守卫看错 dict。裸构 orch 默认 `lambda: False` 保 P11.2.X 之前行为零改。
...
`pyproject.toml` 已设 `asyncio_mode = "auto"`，async 测试不用加 mark。`tests/` 当前 ~100 文件 / ~1300 测（最近一次 `pytest --collect-only` 1283），覆盖 orchestrator / scoring / handoff / MTS / task / artifact / persona / server inbound / room runtime / P11 bg run 等。fixture 共享 `tests/conftest.py`（`build_orch` 裸构 Orchestrator / `SpeakingStubGuest` 走真 speak / `task_inbound_srv` 装真房间 + monkeypatch `_run_ai_chain` no-op）。**复现 bug 优先**，先写失败用例再修。跑全量 `uv run pytest`（~45s），单测 `uv run pytest tests/test_xxx.py -v`。
