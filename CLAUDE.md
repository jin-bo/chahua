...
- **仅 dir-form 才查 manifest，flat-form 跳过**。flat-form md 的 parent 是 `chahua/personas/` 本身；误置一份根级 `persona.toml` 会让所有 flat-form persona 共享同份 `[defaults.guest]` —— 两处守卫同语义不同写法：`admin_room._build_guest_with_manifest_defaults` 内联判定 `md_abs.parent.name != PERSONAS_REL_DIR.name`；`admin_persona._resolve_display_and_summary` 接 `is_dir_form: bool` 形参，由 `discover_personas` 的两段循环（flat `glob('*.md')` vs dir-form `iterdir`）分别传值。**P12.1 起**：内置 5 位（宝总 / 汪小姐 / 范总 / 玲子 / 爷叔）迁到 dir-form + manifest，built-in 不再走 flat 路径；flat-form 仍是合法格式（社区包可继续用）。
...
- **后台 runtime 仅在真有 in-flight 活或挂着 dormant MTS 时存在**。bg turn / handoff drain / bg run 跑完，wrapper finally 调 `_maybe_self_destruct_background_runtime` 自毁。**P8.4 后**：`busy_alive` 把 `has_managed_session()` 也计入 busy —— dormant MTS（drain 收尾不再终结的状态）会让 bg runtime 留活，用户切回还能看到 MTS 状态；正经收尾（`managed_session_stop` / task 终结 / `_aclose_one_runtime` / `MAX_BACKGROUND_ROOMS` 淘汰）都先 `end_managed_session()` 再走 close，故不会泄漏（Codex P8.4 round 2 P2）。切回竞态：`_switch_room` 先翻 `mode=foreground`，自毁判定随即不成立。
...
- **dormant 复活走 drain 路径 + skip chain（P8.4.7）**。`submit_user_message` 入口当 MTS 活 + 队列空 + manager 在场 + manager 不忙时，**pre-enqueue** 一个 manager DELEGATE kickoff + emit `HANDOFF_ENQUEUED` 快照 → 既有 drain 像 `managed_session_start` 路径一样消费它（`<managed_session>` 块经 `_build_winner_blocks` 注入）→ 该路径**跳过** `_run_ai_chain`（与 `_inbound_managed_session_start` 走 `_run_handoff_turn` 不跑 chain 同口径）。**Why**：原 P8.4.1「chain 跑完 → re-drain」依赖 manager scoring 胜出 + 拿到 `<managed_session>` 块，两者都不成立 —— chain scoring 不保证胜出（统计行为）、`<managed_session>` 块是 drain 独占注入点（chain 路径拿不到），dormant 永远卡死无反馈（Codex P8.4 round 1 P2）。manager 不在场 / 正忙 → 不补 kickoff、走常规 chain（MTS 仍 dormant 等用户下次再试或停止）。chain 后 re-drain（`reset_cap=False`）保留：捕获 chain 内被 `@` 强制 / 偶然胜出的 manager 经 hook 自动入队的项。
- **`managed_session_start` 拒收 bg-run busy manager（P8.4.7 / P8.4.9 narrow）**。`_inbound_managed_session_start` 在 ②（manager 在场）和 ③（budget 范围）之间加 ②b：`runtime.guest_in_bg_run(manager_guest)` → NOTICE error 拒开。**Why**：kickoff DELEGATE 会被 drain `_advance_to_runnable_handoff` 的 busy-winner 守卫静默 drop（P11 C12），而 P8.4「队列空→dormant」让 UI 已收到 `managed_session_started` 却永远没 kickoff 接续，MTS 卡死无反馈（Codex round 1 P2）。**P8.4.9 narrow（Codex round 3 P2）**：只查 bg run、不查 `active_guest_names` 全集 —— 末尾 `_enqueue_handoff_and_maybe_start` 会 cancel + drain user/handoff inflight 抢占清场，前台 / handoff 占用 manager 不阻止启动；只有 bg run 是不可抢占的真忙。
- **MTS proposal intercept：summarizer 不查 manager 自己（P8.4.9）**。`ManagedSessionOps._handoff_item_from_proposal` 在 manager 自己的 `speak()` 内跑，`let_speak` 把 manager 标 busy；但被提议的 handoff item 在 manager 这一轮 `speak()` finally 解 busy 后才被 drain 消费。「workers 讨论、我汇总」（panel.summarizer = manager）是 MTS 自治流核心模式 —— 用 `busy - {manager}` 做 summarizer 检查放行；delegate target=manager / panel panelist=manager 仍按全集 busy 拒（语义闭环 + 圆桌应为 worker 间讨论）（Codex round 3 P2）。
...
- **`guest_busy` vs `guest_in_bg_run` 二分（P8.4.9）**。两者数据源不同：`guest_busy(name)` 读 `active_guest_names` 全集（前台 / handoff / bg run 任一来源），是「此刻占用」标志；`guest_in_bg_run(name)` 只查 `agent_runs.values()`，是「不可抢占」标志。**handoff inbound 准入校验（delegate / review / panel / managed_session_start ②b）必须用 `guest_in_bg_run`**：原 P11 C12 用 `guest_busy` 把「正在前台讲话但可被抢占」误判为「不可派活」，破坏 `_enqueue_handoff_and_maybe_start` 的 cancel + drain 抢占契约（Codex round 3 P2）。**drain 自身的 busy-winner 守卫**（`_advance_to_runnable_handoff`）仍用 `active_guest_names` 全集兜底 —— 覆盖 inbound→drain race / MTS auto-enqueue 路径的瞬态。
