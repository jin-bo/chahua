...
- `task_tools.py` — 四个茶客侧 task-aware 工具：`task_list_artifacts`（active 任务 markdown 清单，read-only）/ `task_propose_decision` / `task_propose_open`（这俩 `is_read_only=True` 但**会 emit `TASK_PROPOSAL` envelope** —— 为权限层放行 ≠ 无副作用；提议触发前端"采纳 / 忽略"卡片，写权限仍永远在用户）/ `task_write_artifact`（绕开 agentao `PathPolicy` 让茶客 `./task/<name>` 落盘的官方通道，`is_read_only=False` 让 read-only 模式拦住；name 校验拒 `/` / `\\` / `..` / 前缀 `.`）。
- `task_event_text.py` — 5 个纯函数（`open_task_text` / `close_task_text` / `add_decision_text` / `attach_artifact_text` / `detect_artifacts_text`）单点维护任务事件合成 user 消息的文案（emoji 前缀 📋 / ✅ / ✗ / 📌 / 📎），不允许 handler 内 inline 拼字符串。
