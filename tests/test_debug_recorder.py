...
@pytest.mark.parametrize(
    ...
    [
        ...
        # handoff_delegate / handoff_review / handoff_panel 与 @ 路由同走
        # threshold=None / MENTION 口径；panel turn winners 是 N(+1) 个名字。
        (SCORING_PATH_HANDOFF_DELEGATE, ["A"]),
        (SCORING_PATH_HANDOFF_REVIEW, ["A"]),
        (SCORING_PATH_HANDOFF_PANEL, ["A", "B", "C"]),
    ],
)
def test_mention_and_broadcast_paths(tmp_path, path, winners):
    """确定性路由路径：threshold=None、results kind 全是 ``mention``、
    scoring_path 标签区分单 @ / @all / handoff_delegate / handoff_review / handoff_panel。"""
    rec = TurnRecorder(tmp_path, enabled=True, capture_prompts=False)
    rec.start_turn(
...
def test_handoff_paths_are_whitelisted():
    """P7.1.2 / P7.2.2 / P7.3.2 白名单回归点：``handoff_delegate`` /
    ``handoff_review`` / ``handoff_panel`` 必须在 :data:`VALID_SCORING_PATHS`
    中（否则 :meth:`record_scoring` 会默默降级）。"""
    assert SCORING_PATH_HANDOFF_DELEGATE in VALID_SCORING_PATHS
    assert SCORING_PATH_HANDOFF_REVIEW in VALID_SCORING_PATHS
    assert SCORING_PATH_HANDOFF_PANEL in VALID_SCORING_PATHS
