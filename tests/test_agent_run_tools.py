...
def test_spawn_tools_are_not_read_only() -> None:
...
def test_spawn_tools_register_both() -> None:
...
def test_single_returns_error_when_callback_none() -> None:
    """``start_agent_run`` 槽位是 None（未挂 server）—— 工具返 Error: 不抛。"""
    guest = _FakeGuest("Carol")
    assert guest.start_agent_run is None
    out = _single(guest).execute(target="Bob", instruction="x")
    assert out.startswith("Error:")
    assert "bg run 入口未装" in out


def test_batch_returns_error_when_callback_none() -> None:
    guest = _FakeGuest("Carol")
    out = _batch(guest).execute(runs=[{"target": "Bob", "instruction": "x"}])
    assert out.startswith("Error:")
    assert "bg run 入口未装" in out
...
def test_single_rejects_empty_target() -> None:
    guest = _FakeGuest("Carol")
    guest.start_agent_run = _StubCallback()
    out = _single(guest).execute(target="", instruction="x")
    assert out == "Error: target 必须是非空字符串。"


def test_single_rejects_empty_instruction() -> None:
    guest = _FakeGuest("Carol")
    guest.start_agent_run = _StubCallback()
    out = _single(guest).execute(target="Bob", instruction="   ")
    assert out == "Error: instruction 必须是非空字符串。"


def test_single_rejects_bad_task_id() -> None:
    guest = _FakeGuest("Carol")
    guest.start_agent_run = _StubCallback()
    out = _single(guest).execute(target="Bob", instruction="x", task_id="")
    assert out == "Error: task_id 若给须为非空字符串。"


def test_single_passes_through_callback_error() -> None:
...
def test_batch_rejects_empty_runs() -> None:
    guest = _FakeGuest("Carol")
    guest.start_agent_run = _StubCallback()
    assert _batch(guest).execute(runs=[]) == "Error: runs 必须是非空数组。"


def test_batch_rejects_over_size_cap() -> None:
    guest = _FakeGuest("Carol")
    guest.start_agent_run = _StubCallback()
    runs = [
...
def test_batch_rejects_dup_target_in_same_batch() -> None:
...
def test_batch_rejects_non_object_item() -> None:
    guest = _FakeGuest("Carol")
    guest.start_agent_run = _StubCallback()
    out = _batch(guest).execute(runs=["not-an-object"])
    assert out == "Error: runs[0] 必须是 object。"


def test_batch_rejects_missing_instruction() -> None:
    guest = _FakeGuest("Carol")
    guest.start_agent_run = _StubCallback()
...
def test_single_success_passes_source_guest() -> None:
    guest = _FakeGuest("Carol")
    cb = _StubCallback()
    guest.start_agent_run = cb
    out = _single(guest).execute(
...
def test_single_strips_instruction_whitespace() -> None:
    """``instruction`` strip 前后空白 —— 茶客 LLM 偶尔会带换行。"""
    guest = _FakeGuest("Carol")
    cb = _StubCallback()
    guest.start_agent_run = cb
    _single(guest).execute(target="Bob", instruction="  审一下  \n")
    assert cb.calls[0]["instruction"] == "审一下"


def test_batch_success_returns_all_run_ids() -> None:
    guest = _FakeGuest("Carol")
    cb = _StubCallback()
    guest.start_agent_run = cb
    runs = [
...
def test_batch_partial_success_reports_created() -> None:
...
def test_getter_picks_up_runtime_swap() -> None:
...
def test_getter_handles_unbinding_back_to_none() -> None:
    ...
    guest.start_agent_run = cb
    tool = _single(guest)
    out_ok = tool.execute(target="Bob", instruction="x")
    assert out_ok.startswith("Successfully spawned")

    guest.start_agent_run = None
    out_err = tool.execute(target="Bob", instruction="y")
