...
from chahua.summarizer import SummarySpan, Summarizer
from chahua.tasks_store import TasksStore
from chahua.user_md import USER_SPEAKER_ID, UserConfig

from conftest import NoopScorer


class _StubSummarizer(Summarizer):
    """房间级 summarizer stub —— 允许塞静态 summaries 进 onboarding 输出。"""

    def __init__(self, summaries: list[SummarySpan] | None = None) -> None:
        self._fixed: tuple[SummarySpan, ...] = tuple(summaries or ())
...
def _build_orch(
    ...
    *,
    ...
    orch = Orchestrator(
        ...
        scorer=NoopScorer(),
        summarizer=_StubSummarizer(summaries),
        cursor=GuestCursor(),
    ...
    # 塞两位茶客进 participants，触发 _display_map
...
def test_room_summaries_preserved_in_onboarding(tmp_path: Path):
    ...
    orch, store, _ = _build_orch(tmp_path, summaries=summaries)
...
def test_room_summaries_preserved_when_no_task(tmp_path: Path):
    ...
    orch, _, _ = _build_orch(tmp_path, summaries=summaries)
    ctx = orch._build_context_for("A", task_id=None)
    assert "近期梗概" in ctx
    assert "一些梗概" in ctx
    assert "当前任务" not in ctx
